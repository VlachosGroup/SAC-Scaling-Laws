...
# 
# After simpliying the descriptor space, we discovered [the diffusion scaling law (DLS)](#DLS) based on the most significant descriptor $ (E_{bind})^2/E_c $ :
# 
# DLS - $ E_a \propto (E_{bind})^2/E_c $ 
# 
# The performance of DLS is compared with other ML [models](#models) and across [metals](#metals) and [supports](#supports). 
# 
...
# Plot coefficients matrix
ridge_coef_matrix = rtools.make_coef_matrix(x_feature_nonzero, J_nonzero, n_features, x_secondary_feature_names)
...
# Plot coefficients matrix
OLS_coef_matrix = rtools.make_coef_matrix(x_feature_nonzero, J_nonzero, n_features, x_secondary_feature_names)
...
DSL_RMSE, DSL_r2 =rtools.parity_plot(y, DSL.predict(X_USM), model_name, output_dir, DSL_RMSE_test)
#rtools.plot_coef(USM_coefs, terms, model_name, output_dir)
...
USM_prediction = DSL.predict(X_USM)

# %% [markdown]
# ### Step 6 - Compare models 
# 
# %% [markdown]
# #### Compare the best elastic net model (including lasso and ridge) with DSL <a name="model"></a>
# 
...
# %% [markdown]
# #### DSL performance across support <a name="support"></a>
# 
...
for type_i, ci, label_i in zip(types, color_set, legend_labels):
    ...
    ax.scatter(x_plot[indices],
                ...
                alpha = 0.8,
                s  = 100)
ax.plot([x_plot.min(), x_plot.max()], [USM.predict(X_DSL).min(), DSL.predict(X_DSL).max()], 'k--',  lw=2)
...
# %% [markdown]
# #### DSL performance across metal <a name="metal"></a>
# 
